...
import { UpstreamError, createSession, searchListings } from '../vectara.js';

/** The gate these routes admit turns through. A factory rather than a module
 *  constant so a test can mount the routes with a budget it can exhaust. */
export function createChatGate(maxTurns: number): SessionGate {
  return new SessionGate({ maxTurns });
}

export function mountChatRoutes(app: Express, opts: { maxTurns?: number } = {}): void {
  const gate = createChatGate(opts.maxTurns ?? MAX_TURNS_PER_SESSION);
